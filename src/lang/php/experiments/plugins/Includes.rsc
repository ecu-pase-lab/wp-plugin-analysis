...
data IncludeGuard = unguardedInclude() | guardedInclude(list[Expr] guardPath);
...
public rel[Expr expr, loc at, IncludeGuard guard] includeGuards(System sys) {
	rel[Expr expr, loc at, IncludeGuard guard] res = { };
	bottom-up visit(sys) {
		case i:include(_,_) : {
			parents = getTraversalContextNodes();
			child = parents[0];
			parents = parents[1..];
			list[Expr] guardsSoFar = [ ];
			while (!isEmpty(parents)) {
				guardsSoFar = extractGuardPath(parents[0], child, parents[1..], guardsSoFar);
				child = parents[0];
				parents = parents[1..];
			}
			if (isEmpty(guardsSoFar)) {
				res = res + < i, i@at, unguardedInclude() >;
			} else {
				res = res + < i, i@at, guardedInclude(guardsSoFar) >;
			}
		}
	}
